import React from "react";

const Skills = () => (
  <div className="skills__container">
    <h4>Skills & knowledge</h4>
    <p>
      I am striving to deepen my understanding of JavaScript and to embrace the
      Web Developer ecosystem. <br />
      Beside <em>JavaScript</em>, <em>jQuery</em>, <em>React</em>,{" "}
      <em>Redux</em>, <em>Nodejs</em>, <em>Express</em>, <em>mongoDB</em>, I am
      also getting my hands on <em>Webpack</em>, <em>Babel</em>, <em>Eslint</em>,{" "}
      <em>Prettier</em>, <em>Jest</em> and more... <br />
      I like to collaborate with other developers ( I won the Hacktoberfest
      T-shirt this year ).
    </p>
  </div>
);

export default Skills;
